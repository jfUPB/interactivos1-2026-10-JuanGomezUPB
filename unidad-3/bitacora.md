...
Lo primero que intentamos fue crear un tipo de message para cada uno de los botones (messageA, messageB, messageS), pero como radio.recieve no puede tener un atributo, era como tirar un dado. Así que lo reescribimos como se ve en el código:


Código del micro.bit para enviar 
```py
from microbit import *
import radio
...
while True:
    ...
    if accelerometer.was_gesture('shake'):
        ...
        radio.send("S")
```
Código del micro.bit para recibir
```py
from microbit import *
import utime
import radio
